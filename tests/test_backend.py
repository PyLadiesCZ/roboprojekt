...
from loading import get_board, get_tiles_properties, board_from_data
...
def test_robot_from_dict():
    ...
    """
    robot_description = {'name': 'crazybot', 'coordinates': [0, 1],
                         'lives': 3, 'flags': 0, 'damages': 4, 'power down': False,
                         'direction': 90, 'start coordinates': [0, 1]}
    robot = Robot.from_dict(robot_description)
    assert robot.name == "crazybot"
    assert robot.coordinates == (0, 1)
    assert robot.lives == 3
    assert robot.flags == 0
    assert robot.damages == 4
    assert robot.power_down == False
    assert robot.direction == Direction.E
    assert robot.start_coordinates == (0, 1)
...
def test_state_from_dict():
    ...
    board = board_from_data(data["board"])
    assert state.robots[0].coordinates == (0, 1)
    assert state.robots[1].damages == 4
    assert board[0, 11][0].direction == Direction.N
    assert board[2, 5][0].name == "ground"
